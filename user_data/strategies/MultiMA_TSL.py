...
class MultiMA_TSL(IStrategy):
    ...
    base_nb_candles_sell = IntParameter(
        5, 80, default=20, space='sell', optimize=False)
    base_nb_candles_sell_trima = IntParameter(
        5, 80, default=20, space='sell', optimize=False)
    high_offset_trima = DecimalParameter(
        0.99, 1.1, default=1.012, space='sell', optimize=False)

    base_nb_candles_buy_ema = IntParameter(
        5, 80, default=20, space='buy', optimize=False)
    low_offset_ema = DecimalParameter(
        0.9, 1.1, default=0.958, space='buy', optimize=False)
    ...
    low_offset_trima = DecimalParameter(
        0.9, 0.99, default=0.958, space='buy', optimize=False)
    rsi_buy_trima = IntParameter(
        30, 70, default=61, space='buy', optimize=False)

    base_nb_candles_buy_zema = IntParameter(
        5, 80, default=20, space='buy', optimize=False)
    low_offset_zema = DecimalParameter(
        0.9, 0.99, default=0.958, space='buy', optimize=False)
    rsi_buy_zema = IntParameter(
        30, 70, default=61, space='buy', optimize=False)
    ...
    # Protection
    ewo_low = DecimalParameter(-20.0, -8.0, default=-
                               20.0, space='buy', optimize=False)
    ewo_high = DecimalParameter(
        2.0, 12.0, default=6.0, space='buy', optimize=False)
    ...
    buy_roc_max = DecimalParameter(
        20, 70, default=55, space='buy', optimize=False)
    buy_peak_max = DecimalParameter(
        1, 1.1, default=1.03, decimals=3, space='buy', optimize=False)
    ...
    low_profit_lookback = IntParameter(
        2, 60, default=20, space="protection", optimize=False)
    low_profit_stop_duration = IntParameter(
        12, 200, default=20, space="protection", optimize=False)
    low_profit_min_req = DecimalParameter(
        -0.05, 0.05, default=-0.05, space="protection", decimals=2, optimize=False)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # EWO
        dataframe['ewo'] = EWO(
            dataframe, self.fast_ewo.value, self.slow_ewo.value)

        # RSI
        dataframe['rsi'] = ta.RSI(dataframe, timeperiod=14)
        dataframe['rsi_fast'] = ta.RSI(dataframe, timeperiod=4)

        dataframe["roc_max"] = dataframe["close"].pct_change(
            48).rolling(12).max() * 100
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ema_offset_buy'] = ta.EMA(dataframe, int(
            self.base_nb_candles_buy_ema.value)) * self.low_offset_ema.value
        dataframe['zema_offset_buy'] = zema(dataframe, int(
            self.base_nb_candles_buy_zema.value)) * self.low_offset_zema.value
        dataframe['trima_offset_buy'] = ta.TRIMA(dataframe, int(
            self.base_nb_candles_buy_trima.value)) * self.low_offset_trima.value
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ema_offset_sell'] = ta.EMA(dataframe, int(
            self.base_nb_candles_sell.value)) * self.high_offset_ema.value
        dataframe['trima_offset_sell'] = ta.TRIMA(dataframe, int(
            self.base_nb_candles_sell_trima.value)) * self.high_offset_trima.value
